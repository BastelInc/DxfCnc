...
int CFile::SetState(int iState)
{
  if ((m_State&0x7FFF)==0 && iState) {
    ...
    char File[MAX_PATH];
    {
      ...
      if (ending) {
        /*if      ((strncasecmp(ending,".stl",5)==0))  {
          ...
          //gBastelUtils.WriteProfileString("CNCFile","LastFile",File);
          if (prefs) prefs->set("LastFile",File);
    ...
    extern int load_file(const char *newfile, int ipos);
          ...
          AktLayer=0xFFFFFFFF;
          if (gObjectManager) gObjectManager->FileOpen(File,1);
        }
      }
    }
    ...
    char File[MAX_PATH];
    {
      ...
      if (ending) {
        ...
        if (gObjectManager && gObjectManager->m_InUse==0) {
          if (strcmp(File,gObjectManager->m_FilePfadName)!=0)
            gObjectManager->FileOpen(File,0);
        }
      }
    }
