...
bool Clipper::ExecuteInternal()
{
  ...
  if (succeeded)
  {
    ...
    for (PolyOutList::size_type j = 0; j < m_PolyOuts.size(); ++j)
    {
      OutRec *outRec = m_PolyOuts[j];
    ...
    cInt& Left, cInt& Right)
{
  if (a1 < a2)
  {
    if (b1 < b2) {Left = __max(a1,b1); Right = __min(a2,b2);}
    else {Left = __max(a1,b2); Right = __min(a2,b1);}
  }
  else
  {
    if (b1 < b2) {Left = __max(a2,b1); Right = __min(a1,b2);}
    else {Left = __max(a2,b2); Right = __min(a1,b1);}
  }
  return Left < Right;
}
//------------------------------------------------------------------------------
...
void ClipperOffset::DoOffset(double delta)
{
  ...
  for (int i = 0; i < m_polyNodes.ChildCount(); i++)
  {
    ...
    {
      int k = len - 1;
      for (int j0 = 0; j0 < len; ++j0)
        OffsetPoint(j0, k, node.m_jointype);
      m_destPolys.push_back(m_destPoly);
    }
    else if (node.m_endtype == etClosedLine)
    {
      ...
      for (int j1 = len - 1; j1 > 0; j1--)
        m_normals[j1] = DoublePoint(-m_normals[j1 - 1].X, -m_normals[j1 - 1].Y);
      m_normals[0] = DoublePoint(-n.X, -n.Y);
      k = 0;
      for (int j2 = len - 1; j2 >= 0; j2--)
        OffsetPoint(j2, k, node.m_jointype);
      m_destPolys.push_back(m_destPoly);
    }
    else
    {
      int k = 0;
      for (int j3 = 1; j3 < len - 1; ++j3)
        OffsetPoint(j3, k, node.m_jointype);
      ...
      //re-build m_normals ...
      for (int j0 = len - 1; j0 > 0; j0--)
        m_normals[j0] = DoublePoint(-m_normals[j0 - 1].X, -m_normals[j0 - 1].Y);
      m_normals[0] = DoublePoint(-m_normals[1].X, -m_normals[1].Y);

      k = len - 1;
      for (int j1 = k - 1; j1 > 0; --j1) OffsetPoint(j1, k, node.m_jointype);
