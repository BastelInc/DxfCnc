...
static RECT rcAuswahlSceen = {0,0,0,0};
  ...
  case FL_PUSH :
    ...
    if (Fl::event_x() < (x() + w() - SBWIDTH2) &&
      ...
      if (gMausMode=='s') {
        ...
        if (rcAuswahlSceen.right > rcAuswahlSceen.left) {
          right = rcAuswahlSceen.right;
          left  = rcAuswahlSceen.left;
        } else {
          left  = rcAuswahlSceen.right;
          right = rcAuswahlSceen.left;
        }
        // Top-Down
        if (rcAuswahlSceen.top < rcAuswahlSceen.bottom) {
          top    = rcAuswahlSceen.top;
          bottom = rcAuswahlSceen.bottom;
        } else {
          bottom = rcAuswahlSceen.top;
          top    = rcAuswahlSceen.bottom;
        }
        ...
        if (!(start_ey_ <  top
          ...
          rcAuswahlSceen.left = start_ex_;
          rcAuswahlSceen.top = start_ey_;
          rcAuswahlSceen.right = Fl::event_x();
          rcAuswahlSceen.bottom = Fl::event_y();
  ...
  case FL_DRAG :
    ...
    if (Fl::event_x() < (x() + w() - SBWIDTH2) &&
      ...
      if (gMausMode=='m') {
        ...
        float mod = 0.5 / (float)h();
        rcAuswahl_gl[0] = (float)(rcAuswahlSceen.left-w()/2)    * mod;
        rcAuswahl_gl[1] = (float)(rcAuswahlSceen.right-w()/2)   * mod;
        rcAuswahl_gl[2] = (float)(h()/2 - rcAuswahlSceen.top)   * mod;
        rcAuswahl_gl[3] = (float)(h()/2 - rcAuswahlSceen.bottom)* mod;
        float mx = center[0] / 1.50;
        float my = center[1] / 1.50;
        ...
        rcAuswahl_gl[2] /= Zoom;
        rcAuswahl_gl[3] /= Zoom;
        //printf(" X%0.2f  Y%0.2f  mx%0.2f my%0.2f CX%0.2f CY%0.2f dif %0.3f\n",rcAuswahl_gl[1],rcAuswahl_gl[3],mx,my,center[0],center[1],dif);
        rcAuswahlCNC.left   = glXScreenToCNC(rcAuswahl_gl[0]);
        rcAuswahlCNC.right  = glXScreenToCNC(rcAuswahl_gl[1]);
        rcAuswahlCNC.top    = glYScreenToCNC(rcAuswahl_gl[2]);
        rcAuswahlCNC.bottom = glYScreenToCNC(rcAuswahl_gl[3]);
...
//        RECT rcAuswahlCNC;
        rcAuswahlCNC.left   = glXScreenToCNC(rcAuswahl_gl[0]);
        rcAuswahlCNC.right  = glXScreenToCNC(rcAuswahl_gl[1]);
//        if (rcAuswahl_gl[2]>rcAuswahl_gl[3]) {
        rcAuswahlCNC.top    = glYScreenToCNC(rcAuswahl_gl[2]);
        rcAuswahlCNC.bottom = glYScreenToCNC(rcAuswahl_gl[3]);
//        } else {
//          rcAuswahlCNC.top    = glYScreenToCNC(rcAuswahl_gl[2]);
...
//        }
      ...
      if (Fl::event_button() == FL_LEFT_MOUSE) {
        ...
        if (abs (rcAuswahlSceen.left-rcAuswahlSceen.right)< 3 &&  abs (rcAuswahlSceen.top-rcAuswahlSceen.bottom)< 3 ) {
          //SettingsShow(0xF0);
        } else {
          SettingsShow(0x80);
        }
        //      SetSelection(&rcAuswahlSceen);
#ifdef MOUSESERVICE
        if (gMausMode!='s' && gDreiDFace==0
            && abs (rcAuswahlSceen.left-rcAuswahlSceen.right)< 3 &&  abs (rcAuswahlSceen.top-rcAuswahlSceen.bottom)< 3 ) {
...
//  glLoadName(0);
  ...
  ZOffset    = 0;
  if (Max<0) {
    Max = gObjectManager->MinMaxback- gObjectManager->MinMaxfront;
  ...
  //wrksptop
  wrkspground= (gPa.Anschlag[Zp]/-2) +  gPa.Ref[Zp] ;
#ifndef DREITABLE
  int XPos  = 0;
  int YPos  = 0;
...
//    LeaveCriticalSection(&SerialComm::s_critical_section);
  ...
  if (gBackground) {
    float backg[4];
//#ifndef DREITABLE
//    backg[0] = backg[1] = backg[2] = LightColor[2]/2.0;
//#else
...
//#endif
  ...
  glLoadIdentity();
  float Zoom2 = Zoom * Perspective / 20;//33;
  if (0&&gDreiDFace) {
    gluLookAt(eye[0]/20.0, eye[1]/20.0, eye[2]/20.0,0,0,0, 0, 1, 0);
  } else {
    glTranslatef(center[0]*-1.0/30.0*Perspective,center[1]*-1.0/30.0*Perspective,center[2]/10.0-1.5);// );
//    gAlphaTransp = 0.6 + (eye[2] / 11.0);
//    gGrayOut     = 0.6 + (eye[1] / 11.0);;
  }
  glScalef(Zoom2, Zoom2, Zoom2);
#else
  float Zoom2 = Zoom ;/// Perspective *10.0;
  glLoadIdentity();
  glFrustum(-0.250, .250, -0.250, .250,0.1 + Perspective/50.0,30.0);
  ...
  glTranslatef(center[0]*-1.0/3.0,center[1]*-1.0/3.0,center[2]-1.5);// );
  glScalef(Zoom2, Zoom2, Zoom2);
#endif
  ...
  if (gMausMode=='s') {
    if (rcAuswahlCNC.left != rcAuswahlCNC.right && rcAuswahlCNC.top != rcAuswahlCNC.bottom) {
      ...
      glEnd();
      /*      glLineWidth(2);
            glColor4f(.8,0.8,0.2,1);
            glBegin(GL_LINE_LOOP);
            glVertex3f(rcAuswahl_gl[0],rcAuswahl_gl[2],0.01);
            glVertex3f(rcAuswahl_gl[1],rcAuswahl_gl[2],0.01);
            glVertex3f(rcAuswahl_gl[1],rcAuswahl_gl[3],0.01);
            glVertex3f(rcAuswahl_gl[0],rcAuswahl_gl[3],0.01);
            glEnd();*/
    }
  }
  ...
  if (1) {
    ...
    float  z1 = glZCNCToScreen(ZPos)+0.02;
    glTranslatef(x1,y1,z1);
    //glutSolidCone(0.4,0.5,20,10);
    glutSolidCone(0.010,-0.02,9,3);
    glTranslatef(-x1,-y1,-z1);
    glBegin(GL_LINES);
  ...
  if (gDreiDFace) {
    ...
    float MaterialDickeU = MaterialDicke -0.1;
    float urcleft   = (wrkspleft - XOffset) * glFaktor;
    float urcright  = (wrkspright - XOffset) * glFaktor;
    float urcbottom = (wrkspbottom - YOffset) * glFaktor;
    float urctop    = (wrksptop - YOffset) * glFaktor;
    float wrkspz    = (wrkspground - ZOffset) * glFaktor;
    float Unterlage[8][3]= {
