...
{
  { FL_WHITE,  FL_COURIER_BOLD,      TS }, // A - Plain
  { FL_GREEN,  FL_HELVETICA_ITALIC,  TS }, // B - Line comments
  { FL_GREEN,  FL_HELVETICA_ITALIC,  TS }, // C - Block comments
  { FL_CYAN,   FL_COURIER,           TS }, // D - Strings
  { FL_MAGENTA,FL_COURIER,           TS }, // E - Directives
  { FL_BLUE,   FL_COURIER_BOLD,      TS }, // F - Types
  { FL_YELLOW, FL_COURIER_BOLD,      TS }, // G - Keywords
};
            ...
            int        length)
{
  ...
  for (current = *style, col = 0, last = 0; length > 0; length --, text ++)
  {
    ...
    if (current == 'A')
    {
      ...
      else if (!last || (*text == '_') ||  *temp == '.')

      {
        // Might be a keyword...
        for (temp = text, bufptr = buf;
//             (/*islower((*temp)&255) ||*/ *temp == '_') && bufptr < (buf + sizeof(buf) - 1);
             (isalnum((*temp)&255) || *temp == '_'|| *temp == '.') && bufptr < (buf + sizeof(buf) - 1);
             *bufptr++ = *temp++)
        {
          // nothing
        }
        ...
        if (!isalnum((*temp)&255) && *temp != '_' && *temp != '.')
        {
          *bufptr = '\0';
...
{
public:
  EditorWindow(int X, int Y, int W, int H, const char *L);
  ~EditorWindow();
...
EditorWindow::EditorWindow(int X, int Y, int W, int H, const char *L) : Fl_Double_Window(X,Y,W,H,L)
{
...
Fl_Window* new_view(int X, int Y, int W, int H, const char *L)
{
//#ifdef DEV_TEST
//  EditorWindow* w = new EditorWindow(660, 500, title);
//#else
  EditorWindow* w = new EditorWindow(X,Y,W,H,L);
//#endif // DEV_TEST
  ...
  m->copy(menuitems, w);
#ifdef DEV_TEST
  w->editor = new Fl_Text_Editor(0, 30, W,370);
#else
  w->editor = new Fl_Text_Editor(0, 30, W,H-30);
#endif // DEV_TEST
...
Fl_Window* Editemain(int X, int Y, int W, int H, const char *L)
{
  Fl_Window* EditWindow = 0;
//  if (EditWindow ==0)
//  {
