...
void * IniPortProc()
{
  ...
  if (gpSerialComm) delete gpSerialComm;
...
void GlTimer_CB(void *data)
{
  ...
  if (LookAtTimeOut ) LookAtTimeOut--;
  struct stat statbuf;
  ...
  if (statbuf_mtime != statbuf.st_mtime) {
    if (LookAtTimeOut == 0 && statbuf_mtime!=0) {
      ...
      if (gBlockTab &&  gBlockTab->IsDlgButtonChecked(IDC_AutoUpdate) ) { // gBlockTab->visible()){
  ...
  }
  {
    ...
    float xd =     RotateVal[Xp]-RotateValSoll[Xp];
    float yd =     RotateVal[Yp]-RotateValSoll[Yp];
    float zd =     RotateVal[Zp]-RotateValSoll[Zp];
    if (xd > 180) {
      RotateVal[Xp]  -= ceil(fabs(xd / 360))*360;
      xd =     RotateVal[Xp]-RotateValSoll[Xp];
    } else if (xd < -180) {
      RotateVal[Xp]  += ceil(fabs(xd / 360))*360;
      xd =     RotateVal[Xp]-RotateValSoll[Xp];
    }
    if (yd > 180) {
      RotateVal[Yp]  -= ceil(fabs(yd / 360))*360;
      yd =     RotateVal[Yp]-RotateValSoll[Yp];
    } else if (yd < -180) {
      RotateVal[Yp]  += ceil(fabs(yd / 360))*360;
      yd =     RotateVal[Yp]-RotateValSoll[Yp];
    }
    if (zd > 180) {
      RotateVal[Zp]  -= ceil(fabs(zd / 360))*360;
      zd =     RotateVal[Zp]-RotateValSoll[Zp];
    } else if (zd < -180) {
      RotateVal[Zp]  += ceil(fabs(zd / 360))*360;
      zd =     RotateVal[Zp]-RotateValSoll[Zp];
    }

    float w = sqrt(pow(xd, 2) + pow(yd, 2) + pow(zd, 2));
    float tempo = 2.0 +(w/15.0);
    if (tempo>w) tempo = w;
    if (w>0.1) {
      RotateVal[Xp] -= tempo * xd /w;
      RotateVal[Yp] -= tempo * yd /w;
      RotateVal[Zp] -= tempo * zd /w;
      gObject_redraw=1;
    }
  }
...
//#endif
  ...
  if ((gGcodeTab && gGcodeTab->visible()|| gDxfColorTab->visible()) && !gOnline)  {
    extern int VIRTUALV1;
    extern int VIRTUALV2;
    extern int VIRTUALV3;
    extern int VIRTUALV4;
    extern int VIRTUALTempo;
    extern char gCNC_HALT;
    EnterCriticalSection(&SerialComm::s_critical_section);
    float xd =     gCNCPos[Xp]-VIRTUALV1;
    float yd =     gCNCPos[Yp]-VIRTUALV2;
    float zd =     gCNCPos[Zp]-VIRTUALV3;
    float ad = (gCNCPos[Ap] - VIRTUALV4)/3.0;
    float w;
    ...
    if (w>3) {
      gCNCPos[Xp] -= tempo * xd /w;
      gCNCPos[Yp] -= tempo * yd /w;
      gCNCPos[Zp] -= tempo * zd /w;
      gCNCPos[Ap] -= (int)(tempo * ad / w);
      gCNCCode = 'R';
      gNewPos=1;
      //pWrk3DSheet->redraw();
      if (gPa.Maschine[eDrehachse]) {
        gBlockSimulation->Meissel((float)gCNCPos[Xp],(float)gCNCPos[Ap],(float)gCNCPos[Zp]);
      } else {
        gBlockSimulation->Meissel((float)gCNCPos[Xp],(float)gCNCPos[Yp],(float)gCNCPos[Zp]);
      }
      glDisplay_redraw=1;
    } else {
...
#endif
  if (/*gSpindleOn || */glDisplay_redraw) {
    glDisplay_redraw=0;
    pWrk3DSheet->redraw();
    TestforCNCCommands();
  }
  if (gBlockTab && gBlockTab->visible()) {
    gBlockSimulation->SimulationService();
  }
...
void PickFile_CB(Fl_Widget*, void*)
{
  ...
  default: 								// PICKED FILE
    if ( native.filename() ) {
      if (gObjectManager) gObjectManager->FileOpen(native.filename(),0);
...
int ShowFileExplorer(LPCSTR szpathstr,LPCSTR szExtention)
{
  ...
  } else strncpy(gszExtention,szExtention,sizeof(gszExtention));
...
{
  if (gpSerialComm) SendToCNC('G',0,0,0);
...
void PostMessage_cb(void *p)
{
  ...
  if (msg ->receiver) {
    ...
    if (cb) cb(msg ->receiver,(void*)(long)msg ->Value);
  }
}
//---------------------------------------------------------------
...
#define FONTSIZE   13
  ...
  if (1) {
    ...
    Fl::background(32+30, 31+30, 26+30);
    Fl::background2(26+30,25+30, 20+30);
    Fl::set_color(FL_SELECTION_COLOR,241, 176, 0);
  } else {
    Fl::scheme("xp");
    //Fl::scheme("gleam");
...
//      ToolPart1->type (0);
    ...
    if (gOptionsBox ) {
      ...
      if (ToolHeight < my ) ToolHeight =my;
    ...
    if (SettingsTabs ) {
      ...
      gBlockTab = new CBlockTab(border,yTool3,xTool2,ToolHeight,"7");
      if (gBlockTab ) {
        gBlockTab ->InitWindow(SettingsTabs,0,border,yTool3,xTool2,ToolHeight,0);
        gBlockTab ->end();
        gBlockTab ->size(xTool2,ToolHeight);
      }
      gDXFOptimize = new CDXFOptimize(border,yTool3,xTool2,ToolHeight,"8");
      if (gDXFOptimize ) {
      ...
      gDXFtoCNCTab = new CDXFtoCNCTab(border,yTool3,xTool2,ToolHeight,"9");
      if (gDXFtoCNCTab) {
      ...
      //if (PerspektiveTool) {
      gPerspektive = new CPerspektive(border,yTool3,xTool2,ToolHeight,"10");//-yStatus
      if (gPerspektive ) {
...
//        BusyStatus->box(FL_THIN_DOWN_BOX);
    ...
    if (WorkPart) {
      ...
      if (1) {
        ...
        {
          WorkPart->begin();
          gWrk3DSheet = new DXF3DDisplay(xTool+yTab,MenuHeight,Mainwidth-xTool-xPerspektive-yTab,Mainheight-yStatus-MenuHeight,NULL);
...
//extern void InitFingerTouchService(void);
...
#endif
  {
    ...
    if (SerialInitDelay)SerialInitDelay--;
...
//        if (gWrk3DSheet->visible()) {
      //        ShowFileExplorer("","");
      //    } else {
      ShowFileExplorer("",".dxf;*.gcode;*.stl");
      //  }
#else
      ...
      //} else {
      ShowFileExplorer("home",".dxf;*.gcode;*.stl");
      //}
#endif
#ifdef  WIN32
      if (ScanFileTree!= -1) {
...
#if LONMSG
  if (gLonService) delete gLonService;
#endif
  if (gpSerialComm) {
    SendToCNC('G',0,0,0);
    delete(gpSerialComm);
