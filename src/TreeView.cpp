...
CTreeView::CTreeView(int X, int Y, int W, int H,const char *L):Fl_Scroll(X,Y,W,H,L)
{
  ...
  m_Journalix          = 0;
  m_SetVisibleIndex=-1;
  m_ContextInvalidate=0;
  memset(m_rgIDS,0,sizeof(m_rgIDS));
  m_Busy =0;
  sMaxWidth  = 400;//fl_width("0")*180;
...
void CTVItem::EnumCloseAll()
{
  return ;
  CTVItem* pTVItem1 = m_pChild;
  while(pTVItem1) {
    pTVItem1->ExpandItem(VK_LEFT);
    if (pTVItem1->m_pChild) {
      ((CTVItem*) pTVItem1)->EnumCloseAll();
    }
    pTVItem1 = pTVItem1->m_pNext;
  }
}
//-------------------------------------------------------------
void  CTreeView::CloseAll()
{
  CTVItem* pTVItem1 = (CTVItem* ) m_TVItem;
  if (SelChanging(m_AktIndex,0)) {
    while(pTVItem1) {
      if (pTVItem1->ExpandItem(VK_LEFT)) {
      }
      if (pTVItem1->m_pChild) {
        pTVItem1->EnumCloseAll();
      }
      pTVItem1 = (CTVItem* )pTVItem1->m_pNext;
    }
    EnsureVisible(m_TVItem);
    SelChanged(0);
    scroll_to(0,0);
  }
  redraw();
...
CTVItem::CTVItem(PWNDCONTROLW pWndControl,CTVItem * pParent, DWORD record):
  Fl_Widget(0,testNumItems*TREEITEMHEIGTH,sMaxWidth,//pWndControl->w(),
...
//-------------------------------------------------------------
void CTVItem::EnumFindItem(CTVItem ** pItem,LPCSTR lpStr)
{
  CTVItem* pTVItem1 = m_pChild;
  while((*pItem==NULL) && pTVItem1) {
...
//-------------------------------------------------------------
CTVItem* CTreeView::FindItem(LPCSTR lpStr,CTVItem* pStarTItem )
{
  CTVItem* pTVItem1 = (CTVItem* ) m_TVItem;
...
//-------------------------------------------------------------
void CTVItem::EnumFindItemPart(CTVItem ** pItem,LPCSTR lpStr)
{
  CTVItem* pTVItem1 = m_pChild;
...
//-------------------------------------------------------------
CTVItem* CTreeView::FindItemPart(LPCSTR lpStr,CTVItem* pStarTItem )
{
  CTVItem* pTVItem1 = (CTVItem* ) m_TVItem;
  if (pStarTItem ) pTVItem1 = pStarTItem;
  int len = strlen(lpStr);
...
int CTreeView::SelChanging(int nIndex,unsigned int wParam)
{
  ...
  if (pTVItem) {
    ret = pTVItem->SelChanging(nIndex,wParam);
  }
  if (w()>400) redraw();
  else InvalidateItem(nIndex);
  return ret;
}
//-------------------------------------------------------------
int CTreeView::SelChanged(int nIndex)
{
  CTVItem* pTVItem = GetTVitem(nIndex);
  if (pTVItem) {
    pTVItem->SelChanged(m_AktIndex=nIndex);
  }
  InvalidateItem(nIndex);
  return 1;
}
...
int  CTVItem::SelChanging(int nIndex,unsigned int wParam)
{
  int ret=1;
  m_Selected = 0;
  m_State |= 0x8000;
...
//-------------------------------------------------------------
int  CTVItem::SelChanged(int nIndex)
{
  m_Selected = 1;
  m_State |= 0x8000;
  return 1;
...
//-------------------------------------------------------------
int CTreeView::InsertRootItem(CTVItem *pItem)
{
  int Position=0;
  if (m_TVItem==NULL) {
    ...
    CTVItem * pTVItem1 = m_TVItem;
    while(pTVItem1 && pTVItem1->m_pNext) {
      pTVItem1 = pTVItem1->m_pNext;
    }
    if (pTVItem1) {
...
//-------------------------------------------------------------
// this procedure looks a little bit nicely as the function with the full paramlist;
...
int CTVItem::InsertChildItem(CTVItem **ppFirstChildItem, CTVItem *pItem, int iInsertAfter)
{
  ...
  if (pTVItem1==NULL) {
    *ppFirstChildItem = pItem;
    ...
    if (pTVItemPrev) {
      ix = m_pWndControl->m_pack->find(pTVItemPrev);
//      printf("found ix = %d\n",ix);
//      stimm noch nicht ganz
      m_pWndControl->m_pack->insert(*pItem,ix);
    } else {
//      printf("ix = %d isert %d \n",ix,ix+1+Position);
  ...
  if (m_pChild) {
    if (Hit==VK_RIGHT) {
      ...
      if (m_NumChild && m_pChild && m_bExpand) {
        Collapse();
        //delete (pTVItem->m_pChild);
        //pTVItem->m_pChild=NULL;
      }
    }
    m_State  |= 0x8000;
  ...
  if (pTVItem) {
    ...
    if (SelChanging(m_AktIndex,0)) {
      ...
      if (pTVItem->m_NumChild ) {
        int ident = x()+ 5+ ICONSIZE+ pTVItem->m_Level*ICONSIZE;
        if (pos < ident) {
          if (DoubleClick==0) {
            ...
            pTVItem->OnClick(DoubleClick, pos);
          }
        } else {
          pTVItem->OnClick(DoubleClick, pos);
        }
...
int CTVItem::OnClick(int DoubleClick,int pos)
{
  if (DoubleClick) SetState(1);
...
void CTVItem::draw()
{
  ...
  Fl_Color bg = FL_BACKGROUND_COLOR;// drawbgcolor();
  int ident = 5+/*ICONSIZE+ */m_Level*ICONSIZE;
  int X = x()+ident;
  int Y = y();
  int W = w()-ident;
#ifdef USEICONS
  if (R->ButtonImage_List[m_Image] && m_Image <= imNumImage) {
    R->ButtonImage_List[m_Image]->draw(X+0,  Y+ ((TREEITEMHEIGTH-ICONSIZE)/ 2),ICONSIZE,ICONSIZE,0,0);
  }
#endif
//  if (w() > TreeWidht)  m_pWndControl->m_ContextInvalidate=1;
  ...
  if (0) {
    if (m_NumChild ) {
      ...
      ReseorceInit::TreeViewPics.draw(left,  Y-1,32,44,(image)*32,0);
    } else {
      image = imVert;
    }

    {
      ...
      while  (pItem && level) {
        ...
        level--;
        if (pItem->m_pNext) {
          ReseorceInit::TreeViewPics.draw(left,  Y-1,32,44,0,0);
        }
  ...
  if (m_Image == imFolderClose) {
    ReseorceInit::TreeViewPics.draw(X+0,  Y,16,22,(imFolderClose+(m_bExpand?1:0)) *16,0);
  } else {
    ReseorceInit::TreeViewPics.draw(X+0,  Y,16,22,(m_Image %44)*16,0);
  }
#else
  if (m_Image == imFolderClose) {
    R->TreeViewPics.draw(X+0,  Y-1,32,44,(imFolderClose+(m_bExpand?1:0)) *32,0);
  } else {
    R->TreeViewPics.draw(X+0,  Y-1,32,44,(m_Image %44)*32,0);
  }
  ...
  if (1) {
    if (m_Selected) {
      if (Fl::focus() == m_pWndControl) { // gTreeView) {
        bg = FL_SELECTION_COLOR;
      } else {
        bg = FL_LIGHT3;
      }
//      bg = FL_SELECTION_COLOR;
      fg = FL_BACKGROUND_COLOR;
...
//-------------------------------------------------------------
static int ValidPush=false;

int CTreeView::handle(int event)
{
  static int validMove;
  if (event == FL_NO_EVENT) return(0);
  if (Fl::event_x()+scrollbar.w() > w() || (Fl::event_y()-y()+scrollbar.w()) > h()) {
  ...
  case FL_KEYBOARD: {
    ...
    case FL_Right:
      ...
      break;
    default:
      if (key >= ' ' && key <= 'z'&& !(Fl::event_state() & (FL_SHIFT | FL_CTRL | FL_ALT | FL_META))) {
        char str[20];
        str[0] = key;
        str[1] = '\0';
  ...
  break;
  case FL_PUSH :
    if (Fl::event_x()+scrollbar.w() > w() && Fl::event_x() < w()) {
      return Fl_Scroll::handle(event);
    }
    ValidPush=true;
    ...
    if (ValidPush) {
      if (validMove && Fl::event_button()== FL_LEFT_MOUSE) {
        ...
        if (abs (valx) > abs(valy)) {
          CTVItem *  pTVItem2 = GetTVitem(m_AktIndex+1);
          if (pTVItem2!= NULL) {
            pTVItem2 ->MoveHorizontal(scroll_startx-valx);
            scroll_startx = valx;
          }
        } else {
